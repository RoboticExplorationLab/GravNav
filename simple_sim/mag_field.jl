...
function IGRF13(r_eci,epc::Epoch)
    ...
    # check for year
    if year<2020
        error("Year must be >2020, I've only loaded in the recent coefficients")
    end
...
ratio = 6371.2/r
  ...
  for k = 2:kmx
      ...
      @inbounds begin
          ...
          lm  = ll + l
          one = (tc*IGRF13_gh_coefs[lm] + t*IGRF13_gh_coefs[lm+nc])*rr

          if m != 0
              two   = (tc*IGRF13_gh_coefs[lm+1] + t*IGRF13_gh_coefs[lm+nc+1])*rr
