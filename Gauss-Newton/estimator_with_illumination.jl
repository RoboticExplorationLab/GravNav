...
using LinearAlgebra, Plots, ForwardDiff, Attitude, StaticArrays
...
function measurement(x, t) ####
    ...
    v2 = SVector(x[10],x[11],x[12])

    r3 = SVector(x[13],x[14],x[15])
    v3 = SVector(x[16],x[17],x[18])

    # NOTE THAT this does not rely on velocity, so a dummy variable is used
    v = zeros(3)
    nu_1 = eclipse_conical([r1*dscale;v], r_sun)
    nu_2 = eclipse_conical([r2*dscale;v], r_sun)
    nu_3 = eclipse_conical([r3*dscale;v], r_sun)
...
function generate_data(x0,T,dt,R)
    ...
    for i = 1:(T-1)
        ...
        Y[i] = measurement(X[i], (epc+t*tscale)) + sqrt(R)*randn(m)
    end
    Y[T] = measurement(X[T], (epc+(T-1)*tscale)) +  sqrt(R)*randn(m)
...
# 0.01 σ for illumination sensor noise
illum_σ = 0.001

# chol = sqrt (sorry)
# R = Diagonal([chief_rσ*ones(3);chief_vσ*ones(3);rel_range_σ*ones(3)].^2)
R = Diagonal([chief_rσ*ones(3); chief_vσ*ones(3); rel_range_σ*ones(3); illum_σ*ones(3)].^2)
cholR = sqrt(R)
invcholR = inv(cholR)
...
function residual(x)
    ...
    for i = 1:T
        xt = @view x[idx_x[i]]
        # sensor residual
        r[idx_y[i]] = invcholR*(Y[i] - measurement(xt, epc + (i-1)*dt*tscale))
    end
...
t_vec /= 3600
plt = plot(t_vec, position_error[2,:]*dscale, title = "Relative Position Errors", label = "Target 1")
plt = plot!(t_vec, position_error[3,:]*dscale, label = "Target 2", xlabel = "Time (hours)", ylabel = "Error (m)")
display(plot(plt))
